...
void ASCharacter::BeginPlay()
{
	...
	// Only run if it's being run on the server (either dedicated or a listen server)
...
void ASCharacter::Tick(float DeltaTime)
{
	...
	// When bWantsToZoom is true it picks ZoomedFOV, and when false it picks DefaultFOV
...
void ASCharacter::OnHealthChanged(USHealthComponent* OwningHealthComp, float Health, float HealthDelta, const class UDamageType* DamageType, class AController* InstigatedBy, AActor* DamageCauser)
{
	if (Health <= 0.0f && !bDied)
	{
		...
		// Sets a time until destroy
		SetLifeSpan(10.0f);

		// Play animation is done in the anim BP based on bDied
